...
After features has been extracted from the data, the feature data is validated through Principal Component Analysis (PCA) to determine the quality of recorded data and identify outliers. Thus, the PCA will only be used as a qualitative tool to validate the data. %due to lack of time
...
In \figref{fig:pcasubplot} a PCA performed on feature data from on test subjects is shown. The left plot of the principal components describe the importance of each identified components, and how much of the data they describe. Using only the first three components, $93.8\%$ of the full data set can be described. Only the first three principal components are used in the plot to the left in \figref{fig:pcasubplot}. Here it can be seen that the clusters are easily distinguishable from each other and have no remarkable outliers. Therefore the data is considered good and can be used in the training of the regressors. 
